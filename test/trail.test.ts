...
test("Trail catalogue has 34 unique complete guides with licensed local media and no invented geometry", () => {
  const media = JSON.parse(readFileSync(new URL("../src/trail/media.json", import.meta.url), "utf8"));
  assert.equal(trails.length, 34);
  assert.equal(new Set(trails.map(trail => trail.slug)).size, 34);
...
test("discovery combines filters, matches scenery and handles diacritics and empty results", () => {
  ...
  assert.equal(filterTrails(" ").length, 34);
});
