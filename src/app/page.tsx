...
export default function HomePage() {
  ...
  return <><SiteHeader /><main className="home-app">
    ...
    <section className="home-section"><div className="site-shell"><div className="screen-section-heading"><div><p className="eyebrow">Selected for you</p><h2>Four journeys to start with</h2></div><Link className="text-link" href="/discover">View all journeys</Link></div><div className="compact-route-grid">{editorial.map((route) => <RouteCard key={route.summary.slug} route={route} variant="compact" />)}</div></div></section>
    <section className="home-section"><div className="site-shell"><p className="eyebrow">Browse by experience</p><h2>What belongs outside your window?</h2><div className="category-grid">{categories.map((category) => <Link key={category.slug} href={`/discover/${category.slug}`}><strong>{category.title}</strong><span>{category.routeSlugs.length} {category.routeSlugs.length === 1 ? "journey" : "journeys"}</span><b aria-hidden="true">→</b></Link>)}</div></div></section>
