...
import { LatinAmericaContext } from "@/components/jordypop-collection";
import { MtbFieldNotes } from "@/components/mountain-field-notes";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return ride ? journeyMetadata({ title: ride.name, description: ride.summary, path: `/mtb/${ride.slug}/`, image: mtbMedia[ride.imageKey] }) : {};
}
export default async function MtbDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="rallii-editorial mtb-editorial"><nav className="rallii-container trail-breadcrumb" aria-label="Breadcrumb"><Link href="/">Rallii</Link><span>/</span><Link href="/mtb/">MTB</Link><span>/</span><span>{ride.name}</span></nav>
    ...
    <header className="trail-detail-title"><div><p className="rallii-kicker">{ride.location} / {ride.kind}</p><h1>{ride.name}</h1></div><SaveMtb slug={ride.slug} name={ride.name} /></header><ShareJourney name={ride.name} path={`/mtb/${ride.slug}/`} mode="mtb" slug={ride.slug} />
    ...
    <aside className="trail-guide"><p className="rallii-kicker">Your ride notes</p><h2>What to know.</h2><p>{ride.notes}</p><h3>Season & conditions</h3><p>{ride.season}</p><h3>Bike choice</h3><p>{ride.bike}</p><h3>Map & elevation</h3><p>Use the local operator’s map for route distance, elevation and trail grades. Rallii does not yet provide MTB navigation, elevation profiles or offline maps.</p><a className="rallii-button" data-outbound="commercial_outbound_click" data-mode="mtb" data-route-id={ride.slug} href={ride.sourceUrl} target="_blank" rel="noreferrer">Local maps & riding guidance ↗</a><p className="trail-fact-note">Static destination notes prepared September 2026. Access, passes, uplift and weather must be checked with the local source.</p></aside></div><MtbFieldNotes ride={ride}/>{ride.region === "Latin America" ? <LatinAmericaContext mode="mtb" slug={ride.slug}/> : null}<SeasonConnection mode="mtb" slug={ride.slug}/><OutdoorTripPlanning place={ride.location} mode="mtb" routeId={ride.slug} sourceUrl={ride.sourceUrl} />
    <section className="mtb-pro-note"><div><p className="rallii-kicker">My Rallii</p><h2>A ride can be part of a bigger trip.</h2><p>Save rides and add them to the same trip collections and notes as Rail, Green and Trail.</p></div><Link className="rallii-text-link" href="/my-rallii/">Open My Rallii →</Link></section>
    {nearby.length ? <section className="rallii-section" data-related-journeys><div className="rallii-section-heading"><div><p className="rallii-kicker">More in {ride.region}</p><h2>Keep the wheels turning.</h2></div><Link href="/mtb/">All riding destinations →</Link></div><div className="trail-grid">{nearby.map(item => <MtbCard key={item.slug} ride={item} />)}</div></section> : null}</div>
  </main>;
}
