...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return trail ? journeyMetadata({ title: trail.name, description: trail.whyGo, path: `/trail/${trail.slug}/`, image: trailMedia[trail.imageKey] }) : {};
}
export default async function TrailDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="rallii-editorial trail-detail">
    ...
    <div className="rallii-container"><TrailCredit imageKey={trail.imageKey} />
      <header className="trail-detail-title"><div><p className="rallii-kicker">{trail.destination} / {trail.region}</p><h1>{trail.name}</h1></div><SaveTrail slug={trail.slug} name={trail.name} /></header><ShareJourney name={trail.name} path={`/trail/${trail.slug}/`} mode="trail" slug={trail.slug} />
      ...
      <aside className="trail-guide"><p className="rallii-kicker">Your field notes</p><h2>Know before you go.</h2><p>{trail.guidance}</p><h3>Best time</h3><p>{trail.bestTime}.</p><h3>Check before you leave</h3><p>Static editorial guidance, reviewed {trail.reviewedAt}. This page does not report live conditions, closures or permit availability.</p><a className="rallii-button" data-outbound="commercial_outbound_click" data-mode="trail" data-route-id={trail.slug} href={trail.sourceUrl} target="_blank" rel="noreferrer">Official trail guidance ↗</a><p className="trail-fact-note">Use the land manager’s current map for navigation. Route tracking and offline trail maps are not available yet.</p></aside></div><OutdoorTripPlanning place={trail.destination} mode="trail" routeId={trail.slug} sourceUrl={trail.sourceUrl} />
      <section className="rallii-section" data-related-journeys><div className="rallii-section-heading"><div><p className="rallii-kicker">Keep wandering</p><h2>Another day, another trail.</h2></div><Link href="/trail/">All trails →</Link></div><div className="trail-grid">{related.map(item => <TrailCard key={item.slug} trail={item} />)}</div></section>
    </div>
  </main>;
}
