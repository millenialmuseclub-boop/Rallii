...
export default async function ComparePage({ searchParams }: { searchParams: Promise<{ routes?: string | string[] }> }) { const routes = getAllRoutes(); const selected = parseComparisonRoutes((await searchParams).routes, routes); return <AppScreenShell title="Compare" context="See two journey experiences side by side." backHref="/discover" backLabel="Discover" mediaKey="compare"><CompareJourneys routes={routes} selected={selected} /></AppScreenShell>; }
