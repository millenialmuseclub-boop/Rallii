...
export default function HomePage() {
  ...
  return <><SiteHeader /><main className="home-app">
    <section className="home-dashboard"><div className="site-shell">
      ...
      <nav className="home-action-grid" aria-label="Start with Rallii Rail">{actions.map((action) => action.route ? <Link key={action.href} className="home-action-tile focus-ring" href={action.href}><RouteMedia summary={action.route.summary} variant="card" /><span><small>Rallii Rail tools</small><strong>{action.label}</strong><em>{action.description}</em><b>Open →</b></span></Link> : null)}</nav>
    </div></section>
    <section className="home-section"><div className="site-shell"><p className="eyebrow">Browse by experience</p><h2>What belongs outside your window?</h2><div className="category-grid category-grid--visual">{categories.map((category) => { const cover = getCollectionCover(category, routes); return <Link key={category.slug} href={`/discover/${category.slug}`}>{cover ? <RouteMedia summary={cover.summary} variant="card" /> : null}<span><strong>{category.title}</strong><small>{category.routeSlugs.length} {category.routeSlugs.length === 1 ? "journey" : "journeys"}</small><b aria-hidden="true">Explore →</b></span></Link>; })}</div><aside className="home-pro-note"><div><p className="eyebrow">Rallii Pro</p><strong>One membership. Every way you Rallii.</strong><p>Trip collections and notes across Rail and Green.</p></div><Link className="text-link focus-ring" href="/pro">See Pro</Link></aside></div></section>
