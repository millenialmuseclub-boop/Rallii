...
export default function HomePage() {
  ...
  return <><SiteHeader /><main className="home-app">
    ...
    <section className="home-section"><div className="site-shell"><p className="eyebrow">Browse by experience</p><h2>What belongs outside your window?</h2><div className="category-grid category-grid--visual">{categories.map((category) => { const cover = getCollectionCover(category, routes); return <Link key={category.slug} href={`/discover/${category.slug}`}>{cover ? <RouteMedia summary={cover.summary} variant="card" creditLinks={false} /> : null}<span><strong>{category.title}</strong><small>{category.routeSlugs.length} {category.routeSlugs.length === 1 ? "journey" : "journeys"}</small><b aria-hidden="true">Explore →</b></span></Link>; })}</div><aside className="home-pro-note"><div><p className="eyebrow">Rallii Pro</p><strong>One membership. Every way you Rallii.</strong><p>Trip collections and notes across all five activities.</p></div><Link className="text-link focus-ring" href="/pro">See Pro</Link></aside></div></section>
