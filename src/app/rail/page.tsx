export const metadata = { title: "Rallii Rail — Curated Rail Journeys" };
...
export default function HomePage() {
  ...
  return <><SiteHeader /><main className="home-app">
    <section className="home-dashboard"><div className="site-shell">
      <div className="home-dashboard__identity"><p className="eyebrow">Rallii Rail · Curated rail journeys</p><h1>Know where to sit, what to see, and when to look.</h1><p>Directional guides for remarkable train journeys.</p></div>
      ...
      <nav className="home-action-grid" aria-label="Start with Rallii Rail">{actions.map((action) => action.route ? <Link key={action.href} className="home-action-tile focus-ring" href={action.href}><RouteMedia summary={action.route.summary} variant="card" creditLinks={false} /><span><small>Rallii Rail tools</small><strong>{action.label}</strong><em>{action.description}</em><b>Open →</b></span></Link> : null)}</nav>
    </div></section>
    <section className="home-section"><div className="site-shell"><p className="eyebrow">Browse by experience</p><h2>What belongs outside your window?</h2><div className="category-grid category-grid--visual">{categories.map((category) => { const cover = getCollectionCover(category, routes); return <Link key={category.slug} href={`/discover/${category.slug}`}>{cover ? <RouteMedia summary={cover.summary} variant="card" creditLinks={false} /> : null}<span><strong>{category.title}</strong><small>{category.routeSlugs.length} {category.routeSlugs.length === 1 ? "journey" : "journeys"}</small><b aria-hidden="true">Explore →</b></span></Link>; })}</div><aside className="home-pro-note"><div><p className="eyebrow">My Rallii</p><strong>Every way you Rallii, together.</strong><p>Trip collections and notes across all five activities.</p></div><Link className="text-link focus-ring" href="/my-rallii/">Open My Rallii</Link></aside></div></section>
