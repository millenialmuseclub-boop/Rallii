...
export function DiscoverCatalogue({ courses, destinations, initialSetting = "all" }: { courses: Course[]; destinations: Destination[]; initialSetting?: Setting | "all"; initialPage?: number }) {
  const [query, setQuery] = useBrowseState<string>("rallii:green-browse:query", "");
  const [chapter, setChapter] = useState<Chapter>("Pacific & Hawaii");
  const [setting, setSetting] = useBrowseState<Setting | "all">(`rallii:green-browse:setting:${initialSetting}`, initialSetting);
  const [access, setAccess] = useBrowseState<AccessType | "all">("rallii:green-browse:access", "all");
  const [experience, setExperience] = useBrowseState<Experience | "all">("rallii:green-browse:experience", "all");
  const [destination, setDestination] = useBrowseState<string>("rallii:green-browse:destination", "all");
  const [filtersOpen, setFiltersOpen] = useState(false);
  const [visibleCountRaw, setVisibleCount] = useBrowseState<string>("rallii:green-browse:count", "12");
  const visibleCount = Math.max(12, Number(visibleCountRaw) || 12);
  ...
  useEffect(() => {
    ...
    try { saved = sessionStorage.getItem("rallii-green-region") as Chapter | null; } catch { /* Optional preference. */ }
  ...
  const chooseChapter = (item: Chapter) => { setChapter(item); setVisibleCount("12"); try { sessionStorage.setItem("rallii-green-region", item); } catch { /* Optional preference. */ } };
  ...
  return <section className="catalogue shell" id="catalogue" aria-live="polite">
    ...
    <label className="search"><span className="sr-only">Search courses, places, architects and collections</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search courses or places…"/><button type="button" onClick={() => setQuery("")}>{query ? "Clear" : "Search"}</button></label>
    ...
    {shown.length ? <><div className="atlas-heading"><div><p className="eyebrow">{isRefined ? "Atlas results" : `Chapter ${chapters.indexOf(chapter) + 1} of ${chapters.length}`}</p><h3>{isRefined ? "Courses matching your view" : chapter}</h3></div><span>{shown.length} courses</span></div><div className="course-rail">{shown.slice(0, visibleCount).map((course) => <CourseCard key={course.slug} course={course}/>)}</div>{shown.length > visibleCount ? <button className="primary-button catalogue-more" onClick={() => setVisibleCount(String(visibleCount + 12))}>Show more courses ({shown.length - visibleCount} remaining)</button> : null}</> : <div className="empty-state"><h3>No course fits that view yet.</h3><p>Try a broader place, architect or setting.</p><button type="button" onClick={reset}>Reset atlas</button></div>}
  </section>;
}
