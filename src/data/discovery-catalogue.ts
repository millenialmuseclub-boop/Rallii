...
export interface DiscoveryEntry {id:string;slug:string;name:string;mode:"rail"|"trail"|"mtb"|"snow"|"green";country:string;countries?:string[];region:string;href:string;summary:string;season:string;tags:string[];bestMonths?:number[]}
export const discoveryCatalogue: DiscoveryEntry[] = [
 ...snowDestinations.map(p=>({id:p.id,slug:p.slug,name:p.name,mode:"snow" as const,country:p.country,region:p.region,href:`/snow/${p.slug}/`,summary:p.summary,season:p.season,tags:p.highlights,bestMonths:p.bestMonths})),
 ...
 ...getAllRoutes().map(({summary:p})=>({id:`rail:${p.slug}`,slug:p.slug,name:p.name,mode:"rail" as const,country:p.country,countries:p.countries,region:p.country,href:`/routes/${p.slug}/`,summary:p.shortDescription,season:"Confirm the operator’s calendar",tags:[...p.experienceTags,...p.journeyTypes,...p.bestFor]})),
];
