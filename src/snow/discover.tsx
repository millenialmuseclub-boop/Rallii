...
export function SnowDiscover() {
  ...
  return <main className="rallii-editorial snow-editorial">
    ...
    <section id="snow-catalogue" className="rallii-container rallii-section"><div className="rallii-section-heading"><div><p className="rallii-kicker">The Snow collection</p><h2>Choose your winter.</h2></div><p>Resorts and ski regions, organized for a trip.</p></div>
      ...
      <Link className="rallii-text-link" href="/snow/compare/">Compare mountains →</Link><div className="trail-filters"><label>Country<select aria-label="Snow country" value={country} onChange={event=>update({country:event.target.value})}>{["All",...new Set(snowDestinations.map(item=>item.country))].map(value=><option key={value}>{value}</option>)}</select></label><label>Planning month<select aria-label="Planning month" value={month} onChange={event=>update({month:event.target.value})}><option value="All">All months</option>{[1,2,3,7,8,9].map(value=><option key={value} value={value}>{["","January","February","March","April","May","June","July","August","September"][value]}</option>)}</select></label></div><details className="catalogue-filters"><summary>Filters <span>{[region, kind].filter(value => value !== "All").join(" · ") || "All destinations"}</span></summary><div className="trail-filters"><label>Region<select value={region} onChange={event => update({ region: event.target.value })}>{["All", ...new Set(snowDestinations.map(item => item.region))].map(value => <option key={value}>{value}</option>)}</select></label><label>Type<select value={kind} onChange={event => update({ kind: event.target.value })}>{["All", "Resort", "Ski region"].map(value => <option key={value}>{value}</option>)}</select></label></div></details>
