...
export function SnowDiscover() {
  ...
  return <main className="rallii-editorial snow-editorial">
    <section className="snow-hero"><div className="rallii-container"><p className="rallii-kicker">Rallii / Snow</p><h1>Follow winter.</h1><p>Mountain towns, deep-snow days and twelve places to begin.</p><a className="rallii-button rallii-button-light" href="#snow-catalogue">Explore snow ↓</a></div></section>
    <section id="snow-catalogue" className="rallii-container rallii-section"><div className="rallii-section-heading"><div><p className="rallii-kicker">The Snow collection</p><h2>Choose your winter.</h2></div><p>Resorts and ski regions, organized for a trip.</p></div>
      <details className="catalogue-filters"><summary>Search & filters</summary><form className="trail-filters" onSubmit={event => { event.preventDefault(); update({ q: query }); }}><label className="trail-search">Search snow<input type="search" placeholder="Try Whistler, Japan or alpine" value={query} onChange={event => setQuery(event.target.value)} /></label><label>Region<select value={region} onChange={event => update({ region: event.target.value })}>{["All", ...new Set(snowDestinations.map(item => item.region))].map(value => <option key={value}>{value}</option>)}</select></label><label>Type<select value={kind} onChange={event => update({ kind: event.target.value })}>{["All", "Resort", "Ski region"].map(value => <option key={value}>{value}</option>)}</select></label><button className="rallii-button" type="submit">Search</button></form></details>
