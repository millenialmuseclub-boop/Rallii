...
export interface ExperienceRef { activity: "rail" | "green" | "trail" | "mtb" | "snow"; slug: string }
...
export function parseCollections(raw: string | null): TripCollection[] {
  try {
    ...
    return value.collections.filter((entry): entry is TripCollection => !!entry && typeof entry.id === "string" && typeof entry.name === "string" && typeof entry.notes === "string" && Array.isArray(entry.experiences) && entry.experiences.every((ref: ExperienceRef) => ref && (ref.activity === "rail" || ref.activity === "green" || ref.activity === "trail" || ref.activity === "mtb" || ref.activity === "snow") && typeof ref.slug === "string"));
  } catch { return []; }
}
