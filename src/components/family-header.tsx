...
export function FamilyHeader({ trail = false, mode }: { trail?: boolean; mode?: "MTB" }) {
  return <header className="rallii-header"><div className="rallii-container rallii-header-inner">
    <Link className="rallii-wordmark" href="/" aria-label="Rallii home"><RalliiMark />Rallii{trail || mode ? <span> / {mode ?? "Trail"}</span> : null}</Link>
