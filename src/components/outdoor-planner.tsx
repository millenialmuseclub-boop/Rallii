...
export function OutdoorPlanner<T extends PlannerPlace>({ mode, title, items, fields, matches, render, notes }: {
  ...
  return <main className={`rallii-editorial ${mode}-editorial`}><div className="rallii-container rallii-section outdoor-planner">
    ...
    <details className="catalogue-filters"><summary>Around the trip</summary><p>For the days around your adventure: {jordypopProjects.map((project, index) => <span key={project.id}>{index ? " · " : ""}<a href={project.href} target="_blank" rel="noopener noreferrer">{project.name}</a> — {project.description}</span>)}</p>{mode === "snow" ? <p><Link href="/guides/swiss-winter-by-rail/">Swiss winter by rail →</Link> · <Link href="/routes/glacier-express/">Explore the Glacier Express →</Link></p> : null}</details>
  </div></main>;
}
