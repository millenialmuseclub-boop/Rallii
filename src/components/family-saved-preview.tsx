...
export function FamilySavedPreview() {
  ...
  const { library: snow } = useSnowLibrary();
  const mtbCount = Object.keys(mtb.rides).length;
  const snowCount = Object.keys(snow.places).length;
  ...
  const been = Object.values(mtb.rides).filter(value => value.status === "ridden").length + Object.values(green.courses).filter(value => value === "played").length + Object.values(trail.trails).filter(value => value === "been").length + Object.values(snow.places).filter(value => value.status === "been").length;
  const recentTrails = [...trails.filter(item => trail.trails[item.slug]).map(item => ({ ...item, href: `/trail/${item.slug}/` })), ...mtbDestinations.filter(item => mtb.rides[item.slug]).map(item => ({ ...item, href: `/mtb/${item.slug}/` })), ...snowDestinations.filter(item => snow.places[item.slug]).map(item => ({ ...item, href: `/snow/${item.slug}/` }))].slice(0, 3);
  const total = railCount + greenCount + trailCount + mtbCount + snowCount;
  return <section className="rallii-container rallii-section family-saved-preview"><div><p className="rallii-kicker">My Rallii / Your places</p><h2>Keep a little<br /><em>“one day” list.</em></h2><p>{total ? "The journeys, rounds, trails, rides and snow days you’re making room for." : "A train through the Alps. A round by the sea. A trail you can’t stop thinking about. Start saving your next adventure."}</p><Link className="rallii-text-link" href="/my-rallii/">Open My Rallii →</Link></div>
    <div><div className="family-saved-counts"><Link href="/saved/"><strong>{railCount}</strong><span>Rail journeys</span></Link><Link href="/green/my-green/"><strong>{greenCount}</strong><span>Golf courses</span></Link><Link href="/my-rallii/"><strong>{trailCount}</strong><span>Hiking trails</span></Link><Link href="/my-rallii/"><strong>{mtbCount}</strong><span>MTB rides</span></Link><Link href="/my-rallii/"><strong>{snowCount}</strong><span>Snow places</span></Link></div><p className="family-saved-summary">{total - been} Want to Go <span> / </span> {been} Been</p>{recentTrails.length ? <ul className="family-preview-list">{recentTrails.map(item => <li key={item.slug}><Link href={item.href}>{item.name} <span>→</span></Link></li>)}</ul> : <p className="trail-fact-note">Saved privately on this device.</p>}</div>
  </section>;
}
