...
export function DiscoverRoutes({ routes, initialFilter = "all" }: { routes: RailRoute[]; initialFilter?: DiscoverFilter }) {
  ...
  const [filter, setFilter] = useBrowseState<DiscoverFilter>("rallii:rail-browse:filter", initialFilter);
  const [group, setGroup] = useBrowseState<"regions" | "experiences" | "duration" | "collections">("rallii:rail-browse:group", "regions");
  const [visibleCountRaw, setVisibleCount] = useBrowseState<string>("rallii:rail-browse:count", "8");
  const visibleCount = Math.max(8, Number(visibleCountRaw) || 8);
  const [query, setQuery] = useBrowseState<string>("rallii:rail-browse:query", "");
  ...
  return <section className="discover-catalogue" aria-labelledby="all-journeys-title">
    <div className="screen-section-heading"><div><p className="eyebrow">World rail catalogue</p><h2 id="all-journeys-title">Find your next journey</h2></div><span role="status">{matches.length} {matches.length === 1 ? "route" : "routes"}{filter !== "all" ? ` · ${titleCase(filter)}` : ""}</span></div>
    ...
    {query || filter !== "all" ? <button className="text-link browse-reset" type="button" onClick={() => { setQuery(""); chooseFilter("all"); }}>Clear search & filters</button> : null}
    ...
    {showingCollections ? <JourneyCollections routes={routes} limit={5} compact /> : <>
      ...
      {matches.length > visibleCount ? <button className="action-button catalogue-more" type="button" onClick={() => setVisibleCount(String(visibleCount + 8))}>Show 8 more journeys</button> : null}
