...
export function MyRallii({ experiences }: { experiences: SavedExperience[] }) {
  ...
  const { canUseCollections } = useEntitlements();
  ...
  function save() {
    if (!draft || !canUseCollections || !draft.name.trim()) return;
    try {
      const next = updateCollection(parseCollections(snapshot()), draft, canUseCollections);
  ...
  return <div className="family-workspace">
    <p>Your saved library across all five activities. Places and trip notes stay private to this device.</p>
    ...
    <div className="family-actions" aria-label="Filter saved status">{[["all", "All statuses"], ["want", filter === "mtb" ? "Want to Ride" : "Want to Go"], ["been", filter === "mtb" ? "Ridden" : "Been"], ...(["mtb", "snow", "all"].includes(filter) ? [["favorite", "Favorites"]] : [])].map(([value, label]) => <button key={value} type="button" aria-pressed={statusFilter === value} onClick={() => setStatusFilter(value)}>{label}</button>)}</div>
