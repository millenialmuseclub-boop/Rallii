...
export function CompareJourneys({ routes, selected }: { routes: RailRoute[]; selected: RailRoute[] }) {
  ...
  return <div className="mt-10">
    ...
    {selected.length < 2 ? <section className="empty-state mt-12"><p className="eyebrow">Compare experiences</p><h2 className="mt-2 font-serif text-3xl">{selected.length ? "Choose one more journey to compare" : "Choose two journeys to compare"}</h2><p className="mt-3 text-stone-600">Select from Rallii’s curated journeys. No route is ranked or declared a winner.</p></section> : <>
