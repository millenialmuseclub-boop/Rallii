...
import { LatinAmericaContext } from "./jordypop-collection";
...
import { getGuidesForRoute } from "@/data/journey-guides";
...
export function RoutePage({ route, nextRoutes }: { route: RailRoute; nextRoutes: Array<{ route: RailRoute; reason: string }> }) {
  ...
  const openingSeatGuide = seatRecommendation(route.bestSideSegments, direction);
  const collections = getCollectionsForRoute(summary.slug);
  const guides = getGuidesForRoute(summary.slug);
  const [surface, setSurface] = useState("overview");

  function chooseSurface(next: string) {
    setSurface(next);
    if (next === "best-side" || next === "timeline") experienceEvent(next === "best-side" ? "best_side" : "timeline", { mode: "rail", route_id: summary.slug, status: direction });
  }
  ...
  return <><SiteHeader /><div className="journey-app-bar"><Link href="/discover">← Discover</Link><span>{summary.name}</span><small>{endpoints.origin} → {endpoints.destination}</small></div><main><article>
    <header className={`site-shell scroll-section py-10 sm:py-16 journey-surface${surface === "overview" ? " journey-surface--active" : ""}`} id="overview">
      <div className="route-hero-grid">
        <div className="route-hero-identity"><p className="eyebrow">{summary.country}</p><h1 className="mt-4 font-serif text-5xl leading-none tracking-tight sm:text-7xl">{summary.name}</h1><p className="mt-4 font-serif text-2xl text-stone-600 sm:text-3xl">{endpoints.origin} → {endpoints.destination}</p><div className="route-hero-controls"><button className="direction-button focus-ring" type="button" aria-pressed={direction === "reverse"} aria-label={`Change journey direction to ${endpoints.destination} to ${endpoints.origin}`} onClick={toggleDirection}>Travel {endpoints.destination} → {endpoints.origin}</button>{openingSeatGuide ? <button className="route-hero-seat-guide focus-ring" type="button" onClick={() => chooseSurface("best-side")}><span>Best Side</span><strong>{formatViewSide(openingSeatGuide.side)}</strong><small>Open seat guide →</small></button> : null}</div></div>
        <RouteMedia summary={summary} variant="hero" />
        <div className="route-hero-summary"><dl className="route-essential-facts grid grid-cols-2 border-y border-stone-300"><Essential label="Approx. duration" value={summary.durationLabel ?? formatDuration(summary.durationMinutes)} /><Essential label="Distance" value={`${summary.distanceKm} km`} /><Essential label="Train" value={summary.trainType} /><Essential label="Reservation" value={formatReservation(summary.reservationStatus)} /></dl><p className="mt-6 max-w-xl text-base leading-7 text-stone-600">{summary.shortDescription}</p><JourneyActions routeName={summary.name} routeSlug={summary.slug} rideModeAvailable={route.capabilities.rideMode} direction={direction} /></div>
      </div>
    </header>
    <nav className="route-section-nav" aria-label="Journey workspace">{[{id:"overview",label:"Overview"},{id:"best-side",label:"Best Side"},{id:"map",label:"Map"},{id:"timeline",label:"Timeline"},{id:"practical",label:"Practical"}].map((item)=><button key={item.id} type="button" aria-pressed={surface===item.id} onClick={()=>chooseSurface(item.id)}>{item.label}</button>)}</nav>
    <div className="site-shell pb-28 sm:pb-28">
      <RouteExperience route={route} direction={direction} activeSurface={surface} onShowMap={() => chooseSurface("map")} />
      <RouteIntelligence route={route} /><LatinAmericaContext mode="rail" slug={summary.slug}/>
      ...
      <section className={`section-space scroll-section journey-surface${surface === "practical" ? " journey-surface--active" : ""}`} id="practical" aria-labelledby="practical-title"><p className="eyebrow">Plan the journey</p><h2 id="practical-title" className="mt-2 font-serif text-4xl sm:text-5xl">Practical Information</h2><dl className="mt-8 grid gap-px bg-stone-300 sm:grid-cols-2">{getOperationalInformation(route).map((item) => <OverviewItem key={item.id} term={item.label} detail={item.detail} note={item.status === "changeable" ? "Check current operator information" : undefined} />)}</dl><GearPicks mode="rail" slug={summary.slug}/><div className="mt-7 flex flex-wrap gap-3"><Link className="cta-button" href={routePlanningHref(summary.slug)}>Plan this journey</Link><Link className="action-button" href={routeStaysHref(summary.slug)}>Find stays</Link><Link className="text-link focus-ring self-center" href="/gear">Rail Gear →</Link></div></section>
      {nextRoutes.length ? <section className="section-space route-connections" data-related-journeys aria-labelledby="continue-title"><div className="section-heading"><div><p className="eyebrow">Continue the story</p><h2 id="continue-title" className="mt-2 font-serif text-4xl sm:text-5xl">Related journeys</h2></div><Link className="secondary-link focus-ring" href="/discover">Browse all journeys →</Link></div><div className="mt-8 grid gap-7 lg:grid-cols-2">{nextRoutes.map((item) => <RouteCard key={item.route.summary.slug} route={item.route} relationshipReason={item.reason} variant="compact" />)}</div><div className="connection-links"><div><p className="eyebrow">Keep exploring</p><div className="mt-3 flex flex-wrap gap-2">{collections.map((collection) => <Link className="context-link focus-ring" key={collection.slug} href={`/discover/${collection.slug}`}>{collection.title}</Link>)}{guides.map((guide) => <Link className="context-link focus-ring" key={guide.slug} href={`/guides/${guide.slug}`}>{guide.title}</Link>)}<Link className="context-link focus-ring" href={`/discover?filter=${getDiscoverFilter(route)}`}>{summary.slug === "belfast-derry" ? "Northern Ireland" : summary.country} journeys</Link><Link className="context-link focus-ring" href={`/search?q=${encodeURIComponent(summary.experienceTags[0])}`}>More {summary.experienceTags[0]} journeys</Link></div></div><Link className="cta-button focus-ring" href={buildComparePath(summary.slug, nextRoutes[0]?.route.summary.slug)}>Compare with {nextRoutes[0]?.route.summary.name}</Link></div></section> : null}
