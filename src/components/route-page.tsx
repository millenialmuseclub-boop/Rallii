...
export function RoutePage({ route, nextRoutes }: { route: RailRoute; nextRoutes: Array<{ route: RailRoute; reason: string }> }) {
  ...
  return <><SiteHeader /><div className="journey-app-bar"><Link href="/discover">← Discover</Link><span>{summary.name}</span><small>{endpoints.origin} → {endpoints.destination}</small></div><main><article>
    ...
    <div className="site-shell pb-28 sm:pb-28">
      ...
      <section className={`section-space scroll-section journey-surface${surface === "practical" ? " journey-surface--active" : ""}`} id="practical" aria-labelledby="practical-title"><p className="eyebrow">Plan the journey</p><h2 id="practical-title" className="mt-2 font-serif text-4xl sm:text-5xl">Good to know</h2><div className="practical-accordions">{route.journeyInformation.map((item, index) => <details key={item.id} open={index === 0}><summary>{item.label}<span aria-hidden="true">＋</span></summary><p>{item.detail}</p></details>)}</div><Link className="floating-route-action" href="/plan">Plan this journey</Link></section>
