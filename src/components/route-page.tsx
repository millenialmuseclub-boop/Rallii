...
export function RoutePage({ route, nextRoutes }: { route: RailRoute; nextRoutes: Array<{ route: RailRoute; reason: string }> }) {
  ...
  return <><SiteHeader /><div className="journey-app-bar"><Link href="/discover">← Discover</Link><span>{summary.name}</span><small>{endpoints.origin} → {endpoints.destination}</small><Link className="journey-app-bar__saved" href="/saved">Saved</Link></div><main><article>
    ...
    <div className="site-shell pb-28 sm:pb-28">
      ...
      <section className={`section-space scroll-section journey-surface${surface === "practical" ? " journey-surface--active" : ""}`} id="practical" aria-labelledby="practical-title"><p className="eyebrow">Plan the journey</p><h2 id="practical-title" className="mt-2 font-serif text-4xl sm:text-5xl">Practical Information</h2><dl className="mt-8 grid gap-px bg-stone-300 sm:grid-cols-2">{route.journeyInformation.map((item) => <OverviewItem key={item.id} term={item.label} detail={item.detail} />)}</dl><div className="mt-7 flex flex-wrap gap-3"><Link className="cta-button" href={routePlanningHref(summary.slug)}>Plan this journey</Link><Link className="action-button" href={routeStaysHref(summary.slug)}>Find stays</Link></div></section>
