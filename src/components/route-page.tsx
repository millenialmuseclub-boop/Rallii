...
export function RoutePage({ route, nextRoutes }: { route: RailRoute; nextRoutes: Array<{ route: RailRoute; reason: string }> }) {
  ...
  return <><SiteHeader /><div className="journey-app-bar"><Link href="/discover">← Discover</Link><span>{summary.name}</span><small>{endpoints.origin} → {endpoints.destination}</small></div><main><article>
    ...
    <div className="site-shell pb-28 sm:pb-28">
      ...
      {nextRoutes.length ? <section className="section-space route-connections" aria-labelledby="continue-title"><div className="section-heading"><div><p className="eyebrow">Continue the story</p><h2 id="continue-title" className="mt-2 font-serif text-4xl sm:text-5xl">Related journeys</h2></div><Link className="secondary-link focus-ring" href="/discover">Browse all journeys →</Link></div><div className="mt-8 grid gap-7 lg:grid-cols-2">{nextRoutes.map((item) => <RouteCard key={item.route.summary.slug} route={item.route} relationshipReason={item.reason} variant="compact" />)}</div><div className="connection-links"><div><p className="eyebrow">Keep exploring</p><div className="mt-3 flex flex-wrap gap-2">{collections.map((collection) => <Link className="context-link focus-ring" key={collection.slug} href={`/discover/${collection.slug}`}>{collection.title}</Link>)}{guides.map((guide) => <Link className="context-link focus-ring" key={guide.slug} href={`/guides/${guide.slug}`}>{guide.title}</Link>)}<Link className="context-link focus-ring" href={`/discover?filter=${getDiscoverFilter(route)}`}>{summary.slug === "belfast-derry" ? "Northern Ireland" : summary.country} journeys</Link><Link className="context-link focus-ring" href={`/search?q=${encodeURIComponent(summary.experienceTags[0])}`}>More {summary.experienceTags[0]} journeys</Link></div></div><Link className="cta-button focus-ring" href={buildComparePath(summary.slug, nextRoutes[0]?.route.summary.slug)}>Compare with {nextRoutes[0]?.route.summary.name}</Link></div></section> : null}
