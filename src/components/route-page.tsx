  ...
  return <><SiteHeader /><main><article>
    <header className="site-shell py-10 sm:py-16">
      <div className="grid items-end gap-9 lg:grid-cols-[0.82fr_1.18fr] lg:gap-14">
        ...
        <div className={`route-visual route-visual--hero route-visual--${summary.slug}`} role="img" aria-label={summary.heroImageAlt}><span className="media-note">Journey photography coming soon</span></div>
      </div>
    ...
    <div className="site-shell pb-20 sm:pb-28">
      ...
      <section className="section-space border-t border-stone-300 pt-10" aria-labelledby="sources-title"><h2 id="sources-title" className="text-sm font-semibold">Route & data sources</h2><ul className="mt-4 grid gap-4 text-xs leading-5 text-stone-600 sm:grid-cols-3">{route.sources.map((source) => <li key={source.id}><span className="block uppercase tracking-[0.12em] text-stone-500">{source.category === "railway-map" ? "Railway / map data" : source.category === "operator" ? "Operator information" : source.category === "infrastructure" ? "Infrastructure information" : "Rallii guidance"}</span>{source.url ? <a className="mt-1 inline-block underline decoration-stone-400 underline-offset-4" href={source.url} rel="noreferrer" target="_blank">{source.label}</a> : <span className="mt-1 block font-medium">{source.label}</span>}<p className="mt-1">{source.note}</p></li>)}</ul></section>
    </div>
  </article></main></>;
}
