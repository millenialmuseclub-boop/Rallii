...
export function PlanJourney({ routes, initialRouteSlug }: { routes: RailRoute[]; initialRouteSlug?: string }) {
  return <><section className="plan-start" aria-labelledby="plan-start-title"><p className="eyebrow">Start with the rail journey</p><h2 id="plan-start-title">Build around a route you love.</h2><div className="plan-start__actions"><Link className="cta-button focus-ring" href="/saved">Open saved journeys</Link><Link className="action-button focus-ring" href="/discover">Find a journey</Link></div><p>{routes.length} prepared route guides are available. Rallii helps you choose the experience; reservations remain with the operator.</p></section><PartnerPlanningPanel routes={routes} initialRouteSlug={initialRouteSlug} /></>;
}
