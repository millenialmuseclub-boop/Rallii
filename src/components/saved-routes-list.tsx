...
export function SavedRoutesList({ routes }: { routes: RailRoute[] }) {
  ...
  return <>{saved.length >= 2 ? <Link className="primary-link mt-8" href="/compare">Compare saved journeys <span aria-hidden="true">→</span></Link> : null}<div className="mt-10 grid gap-7 lg:grid-cols-2">{saved.map((route) => <RouteCard key={route.summary.slug} route={route} />)}</div></>;
}
